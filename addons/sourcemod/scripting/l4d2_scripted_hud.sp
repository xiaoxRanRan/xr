...
// 手动添加缺失的原生函数声明
native int SMPlus_GetMedkitBonus();
native int SMPlus_GetMaxMedkitBonus();
native int SMPlus_GetAdrenalineBonus(); 
native int SMPlus_GetMaxAdrenalineBonus(); 
// ====================================================================================================
// Cvar Flags
// ====================================================================================================
...
void GetHUD1_Text(char[] output, int size)
{
	...
	if (g_bhybridScoringAvailable) // [2]
    {
        // 获取健康分、伤害分
        float currentHealthBonus = float(SMPlus_GetHealthBonus());
        float currentDamageBonus = float(SMPlus_GetDamageBonus());
        float maxHealthBonus = float(SMPlus_GetMaxHealthBonus());
        float maxDamageBonus = float(SMPlus_GetMaxDamageBonus());
        float healthBonusPercent = (maxHealthBonus > 0.0) ? (currentHealthBonus / maxHealthBonus * 100.0) : 0.0;
        float damageBonusPercent = (maxDamageBonus > 0.0) ? (currentDamageBonus / maxDamageBonus * 100.0) : 0.0;

        // 获取急救包分数
        float currentMedkitBonus = float(SMPlus_GetMedkitBonus()); // [2]
        float maxMedkitBonus = float(SMPlus_GetMaxMedkitBonus()); // [2]
        float medkitBonusPercent = (maxMedkitBonus > 0.0) ? (currentMedkitBonus / maxMedkitBonus * 100.0) : 0.0;

        // --- 获取药丸和肾上腺素分数并合并 ---
        float currentPillsBonus = float(SMPlus_GetPillsBonus());
        float maxPillsBonus = float(SMPlus_GetMaxPillsBonus());
        float currentAdrenalineBonus = float(SMPlus_GetAdrenalineBonus());
        float maxAdrenalineBonus = float(SMPlus_GetMaxAdrenalineBonus());

        // 合并当前分数和最大分数
        float currentPillsAndAdrenalineBonus = currentPillsBonus + currentAdrenalineBonus;
        float maxPillsAndAdrenalineBonus = maxPillsBonus + maxAdrenalineBonus;
        float pillsAndAdrenalineBonusPercent = (maxPillsAndAdrenalineBonus > 0.0) ? (currentPillsAndAdrenalineBonus / maxPillsAndAdrenalineBonus * 100.0) : 0.0;
        // --- 结束药丸和肾上腺素合并部分 ---

        // 计算总奖励分 (使用合并后的药丸/针剂分数)
        float totalBonus = currentHealthBonus + currentDamageBonus + currentMedkitBonus + currentPillsAndAdrenalineBonus;

        // 更新FormatEx的格式字符串, 使用合并后的 "药/针分"
        // 你可以根据需要调整 "药/针分" 的显示名称
        FormatEx(output, size, "%s\n奖励分: %.0f [HB: %.0f%% | DB: %.0f%% | 药/针分: %.0f / %.0f%% | 包分: %.0f / %.0f%%]",
                 output,
                 totalBonus,
                 healthBonusPercent,
                 damageBonusPercent,
                 currentPillsAndAdrenalineBonus, pillsAndAdrenalineBonusPercent, // 使用合并后的分数和百分比
                 currentMedkitBonus, medkitBonusPercent); // [2]
        }
