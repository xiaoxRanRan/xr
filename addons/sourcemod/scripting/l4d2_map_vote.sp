...
public void OnPluginStart()
{
	...
	RegAdminCmdEx("sm_vpk", Cmd_Reload, ADMFLAG_ROOT);
...
public void OnConfigsExecuted()
{
	...
	RegConsoleCmdEx("sm_maps", Cmd_VoteMap);
	RegConsoleCmdEx("sm_mapvote", Cmd_VoteMap);
	RegConsoleCmdEx("sm_chmap", Cmd_VoteMap);
...
void SetFirstMapString()
{
    ...
    for (SourceKeyValues kvSub = kvRoot.GetFirstTrueSubKey(); kvSub; kvSub = kvSub.GetNextTrueSubKey())
    {
        ...
        if (!kvFirstMap)
        {
            kvFirstMap = kvSub.FindKey("Map");
            
            if (!kvFirstMap)
            {
                FormatEx(buffer, sizeof(buffer), "modes/coop/1/Map");
                kvFirstMap = kvSub.FindKey(buffer);
                
                if (!kvFirstMap)
                {
                    FormatEx(buffer, sizeof(buffer), "modes/versus/1/Map");
                    kvFirstMap = kvSub.FindKey(buffer);
                }
            }
        }
        ...
        LogMessage("[MapVote] Added map: %s -> mission: %s", sFirstMap, buffer);
    }
}
...
void StartVoteMap(int client, const char[] chapter)
{
    ...
    Handle vote = NativeVotes_Create(Vote_Handler, NativeVotesType_Custom_YesNo);
    if (vote == null)
    {
        LogError("Failed to create vote");
        return;
    }

    NativeVotes_SetInitiator(vote, client);
    NativeVotes_SetTitle(vote, "%T", "_VoteDisplayTitle", client, chapter);
    NativeVotes_SetDetails(vote, chapter);
    ...
    if (!NativeVotes_Display(vote, iClients, iPlayerCount, 20))
    {
...
int Vote_Handler(Handle vote, MenuAction action, int param1, int param2)
{
    switch (action)
    {
        ...
        {
            char sDisplay[256];
            NativeVotes_GetTitle(vote, sDisplay, sizeof(sDisplay));
        ...
        case MenuAction_VoteEnd:
        {
            if (param1 == NATIVEVOTES_VOTE_YES) 
            {
                ...
                NativeVotes_GetDetails(vote, sMap, sizeof(sMap));
                
                if (g_cvClearScores.BoolValue)
                    SDKCall(g_hSDKClearTeamScores, g_pTheDirector, true);

                LogMessage("[MapVote] Attempting to change to map: %s", sMap);

                if (g_smFirstMap.GetString(sMap, sMissionName, sizeof(sMissionName)))
                {
                    LogMessage("[MapVote] Found mission name: %s, changing mission", sMissionName);
                    SDKCall(g_hSDKChangeMission, g_pTheDirector, sMissionName);
                }
                else
                {
                    LogMessage("[MapVote] No mission found for map %s, using direct changelevel", sMap);
