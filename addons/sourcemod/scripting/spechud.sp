...
// Boss Spawn Scheme
StringMap hFirstTankSpawningScheme, hSecondTankSpawningScheme;
StringMap hFinaleExceptionMaps;
StringMap hCustomTankScriptMaps;
...
float g_fLastAttackTime[MAXPLAYERS+1][MAXPLAYERS+1];
float g_fLastHittableTime[MAXPLAYERS+1];
...
public void OnPluginStart()
{
    ...
    for (int i = 1; i <= MaxClients; ++i)
    {
        ...
        g_fLastHittableTime[i] = 0.0;
        
        for (int j = 1; j <= MaxClients; ++j)
        {
            g_fLastAttackTime[i][j] = 0.0;
        }
    }
...
void Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
    ...
    {
        if (currentTime - g_fLastAttackTime[attacker][victim] > TANK_ATTACK_COOLDOWN)
        {
            attack.Punch++;
            g_fLastAttackTime[attacker][victim] = currentTime;
        }
    }
    ...
    {
        if (currentTime - g_fLastHittableTime[attacker] > TANK_ATTACK_COOLDOWN)
        {
            attack.Hittable++;
            g_fLastHittableTime[attacker] = currentTime;
        }
    }
...
void Event_PlayerIncapStart(Event event, const char[] name, bool dontBroadcast)
{
    ...
    int attacker = GetClientOfUserId(event.GetInt("attacker"));
    if (!attacker || !IsClientInGame(attacker) || GetClientTeam(attacker) != 3) return;
    if (GetInfectedClass(attacker) != L4D2Infected_Tank) return;
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
    ...
    {
        if (!IsClientInGame(tank) || !IsTank(tank) || !IsPlayerAlive(tank))
            continue;
...
stock int GetTankPassedCount(int tank)
{
    if (GetFeatureStatus(FeatureType_Native, "L4D2Direct_GetTankPassedCount") != FeatureStatus_Unknown)
        return L4D2Direct_GetTankPassedCount();
    else
        return 0;
}