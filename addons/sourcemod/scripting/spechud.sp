...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
    ...
    bool isAlive = IsPlayerAlive(tankClient);
    ...
    // 显示血量信息 - 修改后的逻辑
    if (!IsPlayerAlive(tankClient))
    {
        strcopy(info, sizeof(info), "生命值  : 死亡");
        healthPercent = 0.0; // 死亡时设为0
    }
    else
    {
    ...
    // 显示燃烧状态 - 修正后的逻辑
    if (isAlive && (GetEntityFlags(tankClient) & FL_ONFIRE))
    {
